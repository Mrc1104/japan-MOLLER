...
void QwHelicityCorrelatedFeedback::ProcessOptions(QwOptions &options)
{
	...
	//fIAFB   = options.GetValue<bool>("IA-Feedback");  // Is this just a historical feedback system (ie precursor to HA-IA?)
...
void QwHelicityCorrelatedFeedback::FeedPITASetPoints(){
	...
	if (fPITASlope!=0) {
		...
		// How to they determine which are added or subtracted?
		...
		fPITASetpoint4=fPrevPITASetpoint4 - correction; // even split of +/-
		fPITASetpoint5=fPrevPITASetpoint5 + correction; // other PITA-esque feedbacks
		fPITASetpoint6=fPrevPITASetpoint6 + correction; // appear to alternate
...
void QwHelicityCorrelatedFeedback::FeedPOSXYSetPoints(){
	...
	if (fxU!=0 && fxV!=0 && fyU!=0 && fyV!=0) {
		// fyV, fyU all get loaded from a parameter file
		// and then are never changed
...
void QwHelicityCorrelatedFeedback::AccumulateRunningSum(QwHelicityCorrelatedFeedback &entry){
	...
	void QwHelicityCorrelatedFeedback::GetTargetChargeStat(){
		// Why is this called?
		// Verdict: This is not called because it is relevent to the PITA
		// Feedback. Rather, we are using PITA as our "first class citizen"
		// in the class and all other feedback systems are kinda tossed in...
		// So we call CalculateRunningAverage because here because why the hell not -- mrc
		...
		for(size_t i=0;i<nBPM;i++){
			...
			// not in each loop!
			if (fAsymmetry.RequestExternalValue(vBPM[i],&fTargetParameter)){ // vBPM[i] is a list of strings
				Double_t this_mean =fTargetParameter.GetValue()*1.0e+6;
				Double_t this_width=fTargetParameter.GetValueWidth()*1.0e+6;
				std::pair<Double_t,Double_t> this_pair = std::make_pair(this_mean,this_width);
				
				// this is checking to see if all the bpms were placed into a vector
				// if not, it constructs the vector of bpms
